...
## 1.0.5
2026-9-15
* 新增 Android/iOS 关键字和周边 POI 搜索，支持分页、分类筛选及扩展返回字段，并补充平台差异说明。
* 修复 Android 地图重复销毁、监听器清理和 Flutter 组件异步初始化与销毁之间的竞态。
* 完善普通截图和区域截图的异常处理，销毁地图时完成挂起请求，避免重复回调。
* 加固定位、地理编码及 POI 搜索的异步请求管理和资源释放。
* 修复 Android 地图类型索引边界检查、坐标数值转换和定位纬度校验。
...
## 1.0.2
2026-08-13
* 新增 Android/iOS 按地理区域截图能力，统一输出指定像素尺寸的 PNG。
* 新增区域截图参数校验、超时和单地图并发保护。
